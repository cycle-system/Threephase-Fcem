...
class threeLevelFCEM(BaseEstimator):
    ...
    def __init__(self,kpiParameters = None,kpiStep = None,categoryMapping = None,weightVectorCategories = None,categoryMapping2 = None,weightVectorSCategories = None,weightVectorGoals = None):
    ...
    def predict(self, X, y = None):
        ...
        for x in X :
            ...
            for vector in self.weightVectorSCategories:
                
                categoryEvaluation2.append([]);
            ################################################################
            
            index = 0;
            
            for Scat in computedCategoryEvaluation:
            
                categoryEvaluation2[self.categoryMapping2[index]].append(Scat);
            ...
            computedSCategoryEvaluation = [];
            
            index = 0;
            
            for matrix in categoryEvaluation2:
                
                Scategory = np.array([self.weightVectorSCategories[index]]);
                Scat2 = np.array(matrix);
                
                computedSCategoryEvaluation.append(np.dot(Scategory,Scat2)[0]);
                
                index += 1;
            
            result.append(computedSCategoryEvaluation)
            
            ################################################################
            goal = np.array(self.weightVectorGoals);
            Scategories = computedSCategoryEvaluation;
            
            goalEvaluation = np.dot(goal,Scategories);
